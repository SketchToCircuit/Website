...
function PacketHandler(data, ws) {
    ...
    if (data.PacketId == 1 && clients.get(ws).isAuth != true) { //AuthHandle
        if (data.AuthTicket == undefined) 
            return;
        if (data.Data.token == undefined) 
            return;
        if (!checkAuthTicket(data.AuthTicket)) 
            return;
...
async function verifyGoogleToken(token) { //Google verify tokenconst
    ticket = await o2Client.verifyIdToken({idToken: token, audience: o2id});
    constpayload = ticket.getPayload();
    return payload;
}
